...
def supervisor_node(state: AgentState):
    ...
    if not llm:
        ...
        if not state.get("messages"):
            ...
            elif "agent" in msg or "handshake" in msg: return {"next": "a2a_expert", "topic": "A2A"}
            else: return {"next": "email_expert", "topic": "General"}
        return {"next": "drafter"}

    supervisor_prompt = """You are a supervisor managing a conversation between these workers: 'git_expert', 'email_expert', 'a2a_expert', 'drafter'.
...
- If the user is asking about other agents, handshakes, or previous A2A negotiations and the 'a2a_expert' HAS NOT answered yet, route to 'a2a_expert'.
